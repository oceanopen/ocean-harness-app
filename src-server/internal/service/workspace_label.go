...
// ToggleIssue 给 issue 贴/撕 label（恢复式 upsert，按 (issueId,labelId) 含软删记录查）：
//   - 未删 → 软删取消；
//   - 已删 → 恢复（清 deleted_at）；
//   - 不存在 → 插入。
//
// 完成后返回该 issue 当前的全部 label 列表。
func (svc WorkspaceLabel) ToggleIssue(req *types.WorkspaceLabelToggleIssueRequest) ([]*model.WorkspaceLabel, error) {
	q := query.Use(svc.Orm)
	ilq := q.IssueLabel.WithContext(svc.Context)

	existing, err := ilq.Unscoped().
		Where(q.IssueLabel.IssueID.Eq(req.IssueID), q.IssueLabel.LabelID.Eq(req.LabelID)).First()
	if err == nil {
		if existing.DeletedAt.Valid {
			// 已删 → 恢复。
			existing.DeletedAt = gorm.DeletedAt{}
			if e := ilq.Unscoped().Save(existing); e != nil {
				return nil, e
			}
		} else {
			// 未删 → 软删取消。
			if _, e := ilq.Where(q.IssueLabel.ID.Eq(existing.ID)).Delete(); e != nil {
				return nil, e
			}
		}
	} else if errors.Is(err, gorm.ErrRecordNotFound) {
		// 不存在 → 插入。
		il := &model.IssueLabel{IssueID: req.IssueID, LabelID: req.LabelID}
		if e := ilq.Create(il); e != nil {
			return nil, e
		}
	} else {
		return nil, err
	...
	// 返回该 issue 当前的全部 label（关联未删的 label）。
	issueLabels, e := ilq.Where(q.IssueLabel.IssueID.Eq(req.IssueID)).Find()
	if e != nil {
		return nil, e
	}
	if len(issueLabels) == 0 {
		return []*model.WorkspaceLabel{}, nil // 避免 IN () 非法 SQL
	}
	labelIDs := make([]int, 0, len(issueLabels))
	for _, il := range issueLabels {
		labelIDs = append(labelIDs, il.LabelID)
	}
	return q.WorkspaceLabel.WithContext(svc.Context).
		Where(q.WorkspaceLabel.ID.In(labelIDs...)).
		Order(q.WorkspaceLabel.SortOrder.Asc(), q.WorkspaceLabel.ID.Asc()).
		Find()
}

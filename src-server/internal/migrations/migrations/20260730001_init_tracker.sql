...
--     子表以直接父单数作前缀（t_workspace_projects / t_project_states / t_project_issues /
...
--   - typed 枚举列（state_group_code / is_default / is_draft / priority）用 TEXT NOT NULL 无默认值，
...
-- t_project_states：issue 状态（引用模型，docs/issue.md §3），所属 project。
-- 只存 (state_group_code, state_code)，展示元数据由目录 StateGroupCatalog/StateCatalog 提供（不在数据行冗余）。
-- state_group_code ∈ backlog/unstarted/started/completed/cancelled（completed 组触发 issue.completed_at）。
CREATE TABLE t_project_states (
    id               INTEGER PRIMARY KEY AUTOINCREMENT,
    project_id       INTEGER  NOT NULL,
    workspace_id     INTEGER  NOT NULL,
    state_group_code TEXT     NOT NULL,
...
-- state_id / parent_id / local_repository_id 逻辑指向他表，但不建 DB 外键。

...
-- 开发阶段（wt_init/developing/pr_open/cleanup）由 issue.stateId 在 started 组开发步骤子 state 上推进表达，不存本表。
-- P1 桩（Module G）：startDev 写假路径记录（不真调 git worktree add）、getList 真查作 worktreePath/worktreeId SSOT；
...
CREATE TABLE t_issue_worktrees (
    ...
    base_ref            TEXT     NOT NULL DEFAULT '',    -- 创建时的基准（如 origin/main）
    status              TEXT     NOT NULL,               -- active|stale|removed（typed enum，无默认值）
    last_active_at      DATETIME,                        -- PTY 有输出时回写（P2，可选）

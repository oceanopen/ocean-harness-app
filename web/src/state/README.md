# web/src/state — 前端状态管理（zustand + TanStack Query）
...
以 `tracker/` 为参照，新建 `web/src/state/<domain>/`：

```
web/src/state/<domain>/
...
（参考 `web/src/shared/useConfigValue.ts` 范式）。将来若后端给某域加 `*:changed` 推送事件，

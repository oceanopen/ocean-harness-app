...
// 存活过滤后即当前全部 claude 进程）拿到 claude pid → 沿 `ps -o ppid=` 逐级向上
// 爬父链（enrich.rs classify_terminal 的正向版：它从 claude 向上找宿主 app，
// 此处从 claude 向上找本 app 的 PTY shell pid）→ 任一级 pid 命中目标会话的
// shell pid 即 true。进程树匹配精确到具体终端（多 pane 同 cwd 也能区分），
// 非输出流启发式（特征随版本变、清屏丢状态）。
//
...
/// 某 claude 进程是否在 session_id 的 shell 子进程树内（沿父链向上找 shell pid）。
fn claude_under_shell(claude_pid: u32, shell: u32) -> bool {
...
pub fn claude_running(store: &PtySessionStore, session_id: &str) -> bool {
    ...
    let running = claudes.iter().any(|s| claude_under_shell(s.pid, shell));
...
mod tests {
    ...
    // parent_pid / claude_under_shell 依赖真实进程树，仅 MAX_ANCESTRY_DEPTH 常量
    // 语义与空 store 行为可离线断言；进程匹配逻辑真机验证（任务 5 验证项）。
    ...
    #[test]
    fn claude_under_shell_rejects_zero_and_self_loop() {
        // pid 0 / 自环（ps 异常返回自身）须终止爬取，防死循环。
        assert!(!claude_under_shell(0, 42));
    }
}

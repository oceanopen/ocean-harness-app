 ...
 * 使用 `ocean_harness_lib::` 前缀是因为 lib.rs 被编译为名为
 * `ocean_harness_lib` 的库 crate（在 Cargo.toml 的 [lib] 中定义的 name）。
 *
 * 调用关系：
 *   main.rs → ocean_harness_lib::run() → lib.rs 中的 pub fn run()
 */
fn main() {
    /* 调用 lib.rs 中定义的 run() 函数来启动 Tauri 应用
     * `ocean_harness_lib` 是 crate 名（在 Cargo.toml 的 [lib].name 中定义）
     * `::run()` 是该 crate 中导出的公开函数
     */
    ocean_harness_lib::run();
}

...
we-claude-terminal 的 HTTP 本地服务（Go 实现，位于 `src-go/`），提供系统信息等能力。前端通过 `fetch` 直接调用。Rust 侧进程管理位于 `src-tauri/src/shared/http_server.rs`，前端页面 `src/windows/panel/HttpServerPage.tsx`。
...
| `build` | Rust spawn 编译后的二进制 `go-server-bin` | 由 `pnpm go:build` 产出，随包分发 |

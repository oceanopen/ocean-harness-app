// 编译 src-server 本地 HTTP 服务到 src-tauri/resources/，供 Tauri 打包（build 模式由 Rust spawn）。
//
...
// 本地未设时按宿主平台推断。pnpm 保证脚本以仓库根为 cwd，故 -C src-server / -o ../... 相对路径与原 npm 脚本一致。
...
execFileSync(
  'go',
  ['build', '-C', 'src-server', '-o', `../src-tauri/resources/go-server-bin${exeSuffix}`, './cmd/server'],
  { stdio: 'inherit' },
);

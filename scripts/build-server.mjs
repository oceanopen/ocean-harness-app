...
// 运行模式由 beforeDevCommand/beforeBuildCommand 经 `TAURI_RUN_MODE=... ` 前缀注入（跨平台靠
// pnpm shellEmulator，见 pnpm-workspace.yaml）；Rust 侧
...
// 仓库根：锚定脚本自身位置（scripts/ 的上一级），对调用方 CWD 无假设
// （tauri beforeDevCommand 从根跑、CI 从根跑、手动任意目录跑均一致）。
...
// 运行模式（dev/build）：由 beforeDevCommand/beforeBuildCommand 经 `TAURI_RUN_MODE=... ` 前缀注入
// （跨平台由 pnpm shellEmulator 保证，见 pnpm-workspace.yaml）。

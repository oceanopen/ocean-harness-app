...
// 仅改本地值（保存按钮统一提交），不发请求。
interface ProjectStateSelectProps {
  value: number;
...
function ProjectStateSelect({ value, projectStates, onChange, disabled }: ProjectStateSelectProps) {
  ...
  return (
    <Select
      ...
      renderValue={v => renderState(projectStates.find(s => s.id === v))}
    >
      {projectStates.map(s => (
